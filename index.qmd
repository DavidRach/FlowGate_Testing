...
With the changes to `ggcyto`, there appears to be a residual "ggcyto_GatingSet" class present that when passed to `as.ggplot()` is causing dispatch issues and causing the error out. Similar to what was done in the [initial](https://github.com/RGLab/ggcyto/commit/4e7589c838c3fb8e3f56e467566bef1c1efb1b3e) attempted ggcyto fix, I removed this class designation. Once this is done, `as.ggplot()` works again, and the ShinyApp launches successfully. 

After gates were drawn, and done clicked, additional errors were outputed with colnames not found. This appears to be due to a ggplot2 internal change. When I updated to the form below, the X and Y axis names were retrieved, and new gates were correctly saved to the GatingSet object, and could be visualized as well. 

![](images/applyGateClose.png)

## Unit Tests

Wrote some additional unit test using `testthat` and `vdiffr` for the functions involved. These can be found [here](https://github.com/DavidRach/flowGate/tree/master/tests/testthat)
